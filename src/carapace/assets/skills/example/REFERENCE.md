...
- `SKILL.md` frontmatter `metadata.carapace`: declares the exec-scoped tunnel to `imap.gmail.com:993` and the `example-hello` / `example-node` command aliases; no credentials are required for this demo.
...
5. Keep the underlying raw `uv run --directory ...` or `pnpm --dir ... run ...` commands in `metadata.carapace`, not in the prose body.
